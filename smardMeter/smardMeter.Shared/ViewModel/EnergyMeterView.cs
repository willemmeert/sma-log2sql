...
using WNS = Windows.Networking.Sockets;
...
namespace smardMeter.ViewModel
{
    public partial class EnergyMeterView : Microsoft.Practices.Prism.Mvvm.BindableBase
    {
        public EnergyMeterView()
        {
            ...
            FGProcessStatus = "Red";
            lstDevicesFound = new SCO.ObservableCollection<String>();
            lstDevicesFound.Add(sNone);
            logEMMessages = new SCG.LinkedList<EnergyMeterMessage>();
...
//            PowerPlotModel.Background = OxyPlot.OxyColors.Transparent;
        ...
        public SCO.ObservableCollection<String> lstDevicesFound
        {
            get;
            set;
        }
        ...
        private WNS.DatagramSocket socketUDP;
        private SCG.LinkedList<EnergyMeterMessage> logEMMessages;       // log of messages, need double ended queue to be able to remove from beginning, add to end
        ...
        public async STT.Task<Status> JoinMulticast()
        {
            if (this.SetMulticastAddress())                     // get multicast from Hubpage
            {
                if (this.SetUDPPort())                        // get UDP port from Hubpage
                {
                    ...
                    this.socketUDP = new WNS.DatagramSocket();
                    ...
                    {
                        if (WNS.SocketError.GetStatus(e.HResult) == WNS.SocketErrorStatus.Unknown)
                        {
                            throw;
                        }
